...
do
    ...
    then
        mod="${BASH_REMATCH[2]}"
        worker="worker-$mod"        
        ssh -i /var/jenkins_home/azure -o "StrictHostKeyChecking no" dev@10.0.0.4 "az vm show --name $worker --resource-group cloud-asr-swarm -o table"
        if [ $? -eq 0 ]        
        then
          echo "VM exist" 
        else
          ...
          ssh -i /var/jenkins_home/azure dev@10.0.0.4 'cd ~/cloud-asr; git pull origin secret'
          ssh -i /var/jenkins_home/azure dev@10.0.0.4 docker node update --label-add model=$mod $worker
          workerfile="$worker.yaml"
          ssh -i /var/jenkins_home/azure dev@10.0.0.4 "cd ~/cloud-asr/swarm-file; docker stack deploy -c $workerfile $worker"
        fi 
    fi
done
